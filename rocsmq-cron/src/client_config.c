...
int custom_config (json_object *json, void * p_datastruct) {
	...
	log_message(INFO, "Delay: %d, Ticks: %d",config->delay, config->tick);

	// parse cronjobs if there..
	get_objval(json, TAG_JOBS, &jobs);
	if (jobs != 0) {
		length = json_object_array_length(jobs);
		log_message(INFO, "Having %d number of jobs", length);
		for(i = 0; i < length; i++) {
			job = json_object_array_get_idx(jobs,i);
			log_message(INFO, "  - Parsing job %d", i);
			parse_cronjob(job, &cronjob);
			add_cronjob(&cronjob);
			log_message(INFO, "  - Job added");
		}
	}
